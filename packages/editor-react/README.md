...
The host supplies persistence, image upload, references, templates, and export services. Equation nodes render with KaTeX while preserving LaTeX for editable Word export. Use `<EquationControl editor={editor} />` alongside the canvas to insert equations or edit a selected equation. Invalid input is flagged before replacement; editing and undo use normal document transactions. Untrusted LaTeX commands are disabled and expansion is bounded. Citation nodes preserve library IDs; bibliography rendering belongs to the host. An optional `layout="columns"` enables continuous column flow, **not print pagination**. Page splitting, floating figures, complete venue rules, collaborative editing, and matching PDF/DOCX pagination remain future work.
